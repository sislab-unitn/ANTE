...
class DBUtils:
    ...
    def create_connection(
        db_file: str, timeout: float = 20.0, logger=None
    ) -> sqlite3.Connection:
    ...
    @staticmethod
    def create_table(
        table: str, fields: list[str], conn: sqlite3.Connection, logger=None
    ) -> None:
    ...
    @staticmethod
    def apply_query(
        query: str, conn: sqlite3.Connection, values: list[Any], logger=None
    ) -> None:
        cur = conn.cursor()
        if values is None:
            cur.execute(query)
    ...
    @staticmethod
    def return_query(
        query: str, conn: sqlite3.Connection, values: list[Any] = None, logger=None
    ) -> list:
        cur = conn.cursor()
        if values is None:
            cur.execute(query)
...
if __name__ == "__main__":
    ...
    DBUtils.create_table(
        "test2", ["id INTEGER PRIMARY KEY", "name TEXT NOT NULL"], conn
    )
